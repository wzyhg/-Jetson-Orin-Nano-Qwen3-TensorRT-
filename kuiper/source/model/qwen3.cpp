...
#include <utility>
#include "../op/kernels/cuda/int8_lmhead_argmax_kernel.cuh"
...
namespace {
bool fused_int8_lmhead_enabled() {
  static bool enabled = []() {
    const char* value = std::getenv("KUIPER_FUSED_INT8_LMHEAD");
    return value != nullptr && std::string(value) != "0";
  }();
  return enabled;
}

size_t Quant8Bytes(uint64_t weight_num, int32_t group_size) {
  CHECK_GT(group_size, 0);
  CHECK_EQ(weight_num % static_cast<uint64_t>(group_size), 0);
  return static_cast<size_t>(weight_num + (weight_num / group_size) * sizeof(float));
}

size_t Qwen3Fp32FileSize(const TransformerConfig& config) {
  const uint64_t layer_num = static_cast<uint64_t>(config.layer_num_);
  const uint64_t hidden_dim = static_cast<uint64_t>(config.hidden_dim_);
  const uint64_t dim = static_cast<uint64_t>(config.dim_);
  const uint64_t kv_dim = static_cast<uint64_t>(config.kv_dim_);
  const uint64_t head_size = static_cast<uint64_t>(config.head_size_);
  const uint64_t vocab_size = static_cast<uint64_t>(config.vocab_size_);
  const uint64_t immediate_dim = static_cast<uint64_t>(config.immediate_dim_);

  uint64_t floats = 0;
  floats += (2 * layer_num + 1) * hidden_dim;
  floats += vocab_size * hidden_dim;
  floats += layer_num * hidden_dim * dim;
  floats += layer_num * head_size;
  floats += layer_num * hidden_dim * kv_dim;
  floats += layer_num * head_size;
  floats += layer_num * hidden_dim * kv_dim;
  floats += layer_num * dim * hidden_dim;
  floats += layer_num * hidden_dim * immediate_dim;
  floats += layer_num * immediate_dim * hidden_dim;
  floats += layer_num * immediate_dim * hidden_dim;
  floats += vocab_size * hidden_dim;
  return sizeof(ModelConfig) + static_cast<size_t>(floats * sizeof(float));
}

size_t Qwen3Int8FileSize(const TransformerConfig& config, int32_t group_size) {
  const uint64_t layer_num = static_cast<uint64_t>(config.layer_num_);
  const uint64_t hidden_dim = static_cast<uint64_t>(config.hidden_dim_);
  const uint64_t dim = static_cast<uint64_t>(config.dim_);
  const uint64_t kv_dim = static_cast<uint64_t>(config.kv_dim_);
  ...
  uint64_t fp32_floats = 0;
  fp32_floats += (2 * layer_num + 1) * hidden_dim;
  fp32_floats += vocab_size * hidden_dim;
  fp32_floats += layer_num * head_size;
  fp32_floats += layer_num * head_size;

  size_t bytes = sizeof(ModelConfig) + sizeof(int32_t);
  bytes += static_cast<size_t>(fp32_floats * sizeof(float));
  bytes += Quant8Bytes(layer_num * hidden_dim * dim, group_size);
  bytes += Quant8Bytes(layer_num * hidden_dim * kv_dim, group_size);
  bytes += Quant8Bytes(layer_num * hidden_dim * kv_dim, group_size);
  bytes += Quant8Bytes(layer_num * dim * hidden_dim, group_size);
  bytes += Quant8Bytes(layer_num * hidden_dim * immediate_dim, group_size);
  bytes += Quant8Bytes(layer_num * immediate_dim * hidden_dim, group_size);
  bytes += Quant8Bytes(layer_num * immediate_dim * hidden_dim, group_size);
  bytes += Quant8Bytes(vocab_size * hidden_dim, group_size);
  return bytes;
}
}  // namespace
...
base::Status Qwen3Model::validate_model_file_size(size_t file_size) const {
  const size_t expected_size =
      is_quant_model_ ? Qwen3Int8FileSize(*config_, group_size_) : Qwen3Fp32FileSize(*config_);
  if (file_size != expected_size) {
    return base::error::ModelParseError(
        "The Qwen3 checkpoint size does not match the selected format. Expected " +
        std::to_string(expected_size) + " bytes, got " + std::to_string(file_size) +
        " bytes. Use qwen0.6.bin2 for fp32, or pass --int8 for Q8_0 int8 checkpoints.");
  }
...
base::Status Qwen3Model::forward(const tensor::Tensor& input, const tensor::Tensor& pos_tensor,
                                 int& next) const {
  return forward_internal(input, pos_tensor, next, true);
}

base::Status Qwen3Model::forward_internal(const tensor::Tensor& input,
                                          const tensor::Tensor& pos_tensor, int& next,
  ...
  if (need_logits && is_quant_model_ && device_type_ == base::DeviceType::kDeviceCUDA &&
      fused_int8_lmhead_enabled()) {
    tensor::Tensor hidden_output = get_buffer(ModelBufferType::kForwardOutput);
    hidden_output.reshape({config_->hidden_dim_});
    cudaMemcpyAsync(hidden_output.ptr<float>(), input.ptr<float>(),
                    config_->hidden_dim_ * sizeof(float), cudaMemcpyDeviceToDevice,
...
void Qwen3Model::create_param_quant_layers() {
  ...
  // rmsnorm attention, ffn, final stay fp32.
  ...
  pos += (2 * config_->layer_num_ + 1) * hidden_dim * sizeof(float);

  // embedding stays fp32.
  ...
  pos += config_->vocab_size_ * hidden_dim * sizeof(float);
  ...
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    ...
    pos = pos + dim * hidden_dim + wq->get_scale_num() * sizeof(float);
  }

  // query norm stays fp32.
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    std::shared_ptr<op::RmsNormLayer> rms_norm_layer =
    ...
    rms_norm_layer->set_weight(0, {config_->head_size_}, this->raw_model_data_->weight(pos),
                               cpu_device_type);
    qwen_layers_->rmsnorm_layers_.push_back(rms_norm_layer);
    pos = pos + config_->head_size_ * sizeof(float);
  }

  // key
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    auto wk = std::make_shared<op::MatmulLayer>(device_type_, hidden_dim, kv_dim, true);
    ...
    pos = pos + hidden_dim * kv_dim + wk->get_scale_num() * sizeof(float);
  }
  ...
  // value
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    auto wv = std::make_shared<op::MatmulLayer>(device_type_, hidden_dim, kv_dim, true);
    ...
    pos = pos + kv_dim * hidden_dim + wv->get_scale_num() * sizeof(float);
  }
  ...
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    ...
    pos = pos + dim * hidden_dim + wo->get_scale_num() * sizeof(float);
  }
  ...
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    ...
    pos = pos + hidden_dim * immediate_dim + w1->get_scale_num() * sizeof(float);
  }
  ...
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    ...
    pos = pos + immediate_dim * hidden_dim + w2->get_scale_num() * sizeof(float);
  }
  ...
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    ...
    pos = pos + immediate_dim * hidden_dim + w3->get_scale_num() * sizeof(float);
  }

  auto lm_head = std::make_shared<op::MatmulLayer>(device_type_, config_->vocab_size_,
                                                   config_->hidden_dim_, true);
...
base::Status Qwen3Model::predict(const tensor::Tensor& input, const tensor::Tensor& pos_tensor,
  ...
  auto status = forward_internal(input, pos_tensor, next, !is_prompt);
  if (!status) {
    return status;
  }
...
int32_t Qwen3Model::post_processing(const tensor::Tensor& pos, bool is_prompt) const {
  if (is_quant_model_ && !is_prompt && device_type_ == base::DeviceType::kDeviceCUDA &&
      fused_int8_lmhead_enabled()) {
    const auto& cls_layer = std::dynamic_pointer_cast<op::MatmulLayer>(qwen_layers_->cls_layer_);
    if (cls_layer && cls_layer->is_quant_layer()) {
      tensor::Tensor input = get_buffer(ModelBufferType::kForwardOutput);
      input.reshape({config_->hidden_dim_});
      const auto& norm = qwen_layers_->rmsnorm_layers_.at(2 * config_->layer_num_);
      CHECK_NE(norm, nullptr);
      STATUS_CHECK(norm->forward(input, input));
      return static_cast<int32_t>(kernel::int8_lmhead_argmax_kernel_cu(
          input.ptr<float>(), cls_layer->get_weight(0).ptr<int8_t>(),
          cls_layer->scales().ptr<float>(), cls_layer->group_size(), cls_layer->dim1(),
          cls_layer->dim0(), cuda_config_ ? cuda_config_->stream : nullptr));
    }
  }
